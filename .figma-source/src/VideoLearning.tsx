    ...
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px' }}>
      ...
      {tab === 'url' ? (
        ...
        <div
          ...
          style={{
            background: dragging ? '#FFEBEE' : 'white',
            border: `2px dashed ${dragging ? '#E53935' : '#D8D5CC'}`,
          ...
          onClick={onImport}
        >
          <div style={{ color: dragging ? '#E53935' : '#B5B0A3', transition: 'color 0.2s ease' }}>
            <UploadIcon size={52} />
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 17, fontWeight: 700, color: '#3D3A33', marginBottom: 6 }}>Kéo và thả video vào đây</div>
            <div style={{ fontSize: 13, color: 'rgba(234,234,224,0.45)' }}>Hỗ trợ MP4, MOV, MKV, AVI · tối đa 2GB</div>
          </div>
          <button style={{ padding: '10px 28px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, fontSize: 14, fontWeight: 600, color: '#3D3A33', cursor: 'pointer' }}>
            Chọn tệp
          </button>
        </div>
      )}
