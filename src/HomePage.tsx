...
type FeaturedCourse = {
  id: number; lang: string; langColor: string; title: string; instructor: string;
  level: string; lessons: number; hours: number; rating: number; students: string;
  image: string; tags: string[]; badge: string; badgeColor: string;
}

// Chỉ được nạp sau khi quản trị viên tạo khóa học.
const COURSES: FeaturedCourse[] = []

const STATS = [
  { value: '14+', label: 'Ngôn ngữ' },
  { value: '0', label: 'Học viên' },
  { value: '0', label: 'Bài học' },
  { value: 'Chưa có', label: 'Hài lòng' },
]
    ...
    <div style={{ background: '#09090F', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 1680, margin: '0 auto', position: 'relative' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(620px, 0.95fr)', gap: 96, alignItems: 'center' }}>

            {/* Left: text */}
            <div style={{ animation: 'fadeInUp 0.5s ease both' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
                <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#FF4D6D', animation: 'pulseDot 2s infinite' }}/>
                <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 13, fontWeight: 600, letterSpacing: '0.13em', textTransform: 'uppercase', color: 'rgba(234,234,224,0.52)' }}>
              ...
              <h1 style={{
                ...
                fontSize: 'clamp(44px, 4.8vw, 72px)', lineHeight: 1.06,
              ...
              <p style={{ fontSize: 18, lineHeight: 1.65, color: 'rgba(234,234,224,0.5)', marginBottom: 36, maxWidth: 530 }}>
                AI phân tích từ vựng chuyên sâu, học qua video thực tế, và lộ trình cá nhân hóa cho 14+ ngôn ngữ.
              </p>

              <div style={{ display: 'flex', gap: 12, marginBottom: 36, flexWrap: 'wrap' }}>
                <button className="btn-primary" style={{ fontSize: 16, padding: '14px 28px' }} onClick={() => setPage('vocabulary')}>
                  <ZapIcon size={15}/> Bắt đầu học miễn phí
                </button>
                <button className="btn-secondary" style={{ fontSize: 16, padding: '13px 26px' }} onClick={() => setPage('video')}>
              ...
              <div style={{ display: 'flex', gap: 28, flexWrap: 'wrap' }}>
                {STATS.map(s => (
                  <div key={s.label}>
                    <div style={{ fontFamily: 'Fraunces, serif', fontSize: 26, fontWeight: 600, color: '#EAEAE0', lineHeight: 1 }}>{s.value}</div>
                    <div style={{ fontSize: 13, color: 'rgba(234,234,224,0.4)', marginTop: 3 }}>{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'fadeInUp 0.5s 0.1s ease both' }}>
              ...
              {activeLangs.length === 0 && (
                <div style={{ minHeight: 132, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', border: '1px dashed rgba(255,255,255,0.13)', borderRadius: 14, color: 'rgba(234,234,224,0.4)', fontSize: 13, textAlign: 'center' }}>
                  Chưa có ngôn ngữ đang học. Hãy chọn một ngôn ngữ để bắt đầu.
                </div>
              )}
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 13.5, fontWeight: 600, color: '#EAEAE0' }}>0 ngày liên tiếp</div>
                    <div style={{ fontSize: 11.5, color: 'rgba(234,234,224,0.4)' }}>Bắt đầu chuỗi học tập của bạn</div>
                  </div>
                </div>
      ...
      <div style={{ maxWidth: 1680, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ padding: '68px 0 0' }}>
          ...
          <div style={{ minHeight: 178, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px dashed rgba(255,255,255,0.12)', borderRadius: 14, color: 'rgba(234,234,224,0.4)', fontSize: 14 }}>
